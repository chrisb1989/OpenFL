...
Install tenacity via pip before running this script. Also, make sure you're using a 0.025mm vertical lift print profile.
'''
...
while blockNum <= lastBlock:
        ...
        if str(layer[6]) != '0x03 ZMove '+str(2000):
                try:
                        ...
                        break
        if str(layer[9]) != '0x03 ZMove '+str(-1990):
                try:
                        layer[9] = FLP.ZMove(usteps=(-1990))
                except:
                        break
        writeLayer()
